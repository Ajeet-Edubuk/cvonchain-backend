...
export const createTreasuryWallet = async (req, res) => {
  try {
...
export const executeRawTransaction = async(req,res)=>
{
    try {
...
export const getBulkOrderDetails  = async(req, res)=>{
    try {
        ...
        return res.status(200).json({
          ...
          const filterData = response.data.data.executed.filter((tx)=>tx.bulk_order_id===bulkOrderId)
          if(filterData)
          {
          return res.status(200).json({
            sucess:true,
